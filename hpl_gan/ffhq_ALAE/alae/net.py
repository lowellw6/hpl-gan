...
import numpy as np
import hpl_gan.ffhq_ALAE.alae.lreq as ln
import math
from hpl_gan.ffhq_ALAE.alae.registry import *
    ...
    def forward(self, x, s1, s2, noise):
        ...
        if noise:
            if noise == 'batch_constant':
                ...
                x = torch.addcmul(x, value=1.0, tensor1=self.noise_weight_1,
                                  tensor2=torch.randn([x.shape[0], 1, x.shape[2], x.shape[3]]).to(x.device))  #@ fixed to device
        else:
...
class Encoder_old(nn.Module):
    ...
    def encode(self, x, lod):
        styles = torch.zeros(x.shape[0], 1, self.latent_size).to("cuda:1") #@ fixed this to work
